...
import { averagetimespent, successrateofnavigations, topvisitedPOIs, highestobstacles, mostreroutingrequests } from "../../../services/zonesApi";
import { navigationLogs } from "../../../services/zonesApi";
...
const TestDeviceStatus = () => {
  ...
  // --- drawChartInPDF and generatePDF functions unchanged ---
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      pdf.setTextColor(33, 33, 33);
      pdf.setFont('helvetica', 'bold');
      pdf.text("Obstacle Summary by Zone", margin, yPos2);
      ...
      pdf.setTextColor(33, 33, 33);
      pdf.setFont('helvetica', 'bold');
      pdf.text("Key Insights & Recommendations", margin, yPos2);

      yPos2 += 15;
      pdf.setFontSize(12);
      pdf.setFont('helvetica', 'bold');
      pdf.text("Key Navigation Insights:", margin, yPos2);
      ...
      insights.forEach((insight, index) => {
        pdf.text(insight, margin + 5, yPos2 + 10 + (index * 7));
      });

      yPos2 += 45;
      pdf.setFontSize(12);
      pdf.setFont('helvetica', 'bold');
      pdf.text("Recommendations:", margin, yPos2);
      ...
      recommendations.forEach((rec, index) => {
        pdf.text(rec, margin + 5, yPos2 + 10 + (index * 7));
      });
    ...
    return (
      <div className="bg-black min-h-screen w-full flex flex-col items-center justify-center">
        ...
        <div className="text-white text-center mt-6 text-lg animate-pulse">
          {generatingPDF ? "Génération du rapport PDF..." : "Chargement des statistiques de navigation..."}
        </div>
      </div>
    );
  }
